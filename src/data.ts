...
export function getEnglishOptions(grade: GradeLevel | ''): string[] {
  if (!grade) return [];
  if (['ז', 'ח'].includes(grade)) {
    return ['דוברי אנגלית', 'א', 'ב', 'ג'];
  } else if (grade === 'ט') {
    return ['דוברי אנגלית', 'א', 'ב', "ג' - יעל", "ג' - מימי"];
  } else if (['י', 'י"א'].includes(grade)) {
    return ['דוברי אנגלית', '5 יח"ל - מואץ', '5 יח"ל', '4 יח"ל', '3-4 יח"ל'];
  } else {
    return ['דוברי אנגלית', '5 יח"ל', '4 יח"ל', '3-4 יח"ל'];
  }
}
    ...
    if (book.associations && book.associations.length > 0) {
      return book.associations.some(assoc => {
        ...
        if (assoc.subjectType === 'אנגלית') {
          ...
          if (selection.englishGroup === '5 יח"ל - מואץ' && assoc.stream === '5 יח"ל') return true;
          if (selection.grade === 'ט') {
    ...
    if (hasEnglishRule) {
      ...
      if (selection.englishGroup) {
        if (book.english.includes(selection.englishGroup)) {
          isMatch = true;
        } else if (selection.englishGroup === '5 יח"ל - מואץ' && book.english.includes('5 יח"ל')) {
          isMatch = true;
        } else if (selection.grade === 'ט' && (selection.englishGroup === "ג' - יעל" || selection.englishGroup === "ג' - מימי") && book.english.includes("ג'")) {
          isMatch = true;
        }
      }
      if (!isMatch) {
